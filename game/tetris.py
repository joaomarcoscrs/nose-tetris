import pygame
import random
from .settings import initialize_settings, GRID_FACTOR
...
class Tetris:
    ...
    def clear_lines(self):
        ...
        while len(new_grid) < self.screen_height // self.grid_size:
            new_grid.insert(0, [0 for _ in range(GRID_FACTOR)])
        self.grid = new_grid
        ...
        for y, row in enumerate(self.current_shape):
            for x, cell in enumerate(row):
                if cell == 1:
                    ...
                    if new_x < 0 or new_x >= GRID_FACTOR or new_y >= self.screen_height // self.grid_size or self.grid[new_y][new_x] != 0:
                        return False
        return True
